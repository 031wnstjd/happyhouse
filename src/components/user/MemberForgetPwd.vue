    ...
    >
      ...
      <div class="container z-index-2 position-relative">
        <div class="row align-items-center vh-100">
          <div
            class="col-lg-4 pb-4 pb-lg-13 pt-lg-3 me-auto col-md-6 z-index-2"
          >
            <div>
              <h3 class="mb-2">비밀번호를 잊어버리셨나요?</h3>
              <p class="mb-4 text-muted">
                입력하신 이메일 주소로 임시비밀번호가 전송됩니다
              </p>
              <div>
                <form class="needs-validation" onsubmit="return false;">
                  ...
                  <div class="small text-danger mb-2" v-if="!valid">
                    가입되지 않은 아이디 또는 이메일입니다
                  </div>
            ...
            <div class="row align-items-end position-relative">
              ...
              <div class="col-lg-6 col-xl-5 text-lg-end">
                ...
                >
                  ...
                  <span class="btn-rise-text">GO!</span></a
                >
              </div>
            </div>
          </div>
...
<script>
import { findPwd, findPwdCheck } from "@/api/member";
...
export default {
  ...
  methods: {
    async onSubmit() {
      await findPwdCheck(this.form, (response) => {
        if (response.data === "success") {
          // id로 email을 찾을 수 있을 때만 비밀번호찾기 로직 실행
          swal({
            title: "비밀번호 찾기 완료!",
            text: this.form.email + "을 확인하세요",
            icon: "success",
            button: "확인",
          });
          findPwd(this.form, (response) => {
            console.log("비밀번호 찾기 성공");
            if (response.data === "success") {
              // this.valid = true;
              this.isChangedPwd = true;
            } else {
              console.log("유효성 검증 성공했지만 에러 발생");
              // this.valid = false;
              // this.isChangedPwd = false;
            }
          });
        } else {
