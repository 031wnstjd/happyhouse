<template>
  <div class="container mt-15 mb-20">
    ...
    <div class="d-grid gap-2 d-md-flex justify-content-md-end">
      <button
        type="button"
        class="btn btn-outline-primary mb-2 col-mg-1"
        @click="moveWrite()"
    ...
    >
      <thead style="background-color: #6f98ff; border: none">
        <tr style="color: white">
          <th class="text-center w-10">번호</th>
...
export default {
  ...
  methods: {
    moveWrite() {
      this.$router.push({ name: "boardRegister" }).catch(() => {});
    },
    rowClick(item) {
      this.$router.push({ path: `/board/detail/${item.articleno}` });
    },
  },
  computed: {
